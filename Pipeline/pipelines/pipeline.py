...
value of default (None if unspecified). `section' defaults to General."""
    ...
    def createjobs(self ):
        ...
        ncpu = self.ncpu
        for f in files:
            if os.path.isdir(f): # for cases of pair_end fastq where each sample has its own directory containing two fastq files
                print(f)
...
{} {}/{} {} {}
date
""".format(self.pipe,self.email,ncpu,mem,self.runtime,self.outdir,jobname,self.qos,self.pipe, self.indir,f, self.outdir,ncpu)
                                    )
